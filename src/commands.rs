...
pub async fn shell_task() {
    ...
    loop {
        ...
        if crate::commands::COMMAND_PENDING.swap(false, core::sync::atomic::Ordering::Acquire) {
            ...
            if let Some(cmd_vec) = cmd_opt {
                if !cmd_vec.is_empty() && !cmd_vec[0].trim().is_empty() {
                    match crate::commands::run_cmd(cmd_vec) {
                        Ok(command_future) => {
                            // Await the command future!
                            match command_future.await {
                                ...
                                Err(error) => crate::println!("Error: {}", error.error_code),
                            }
                        }
                        Err(error) => crate::println!("Error: {}", error.error_code),
                    }
                ...
                match fs.find_file(&filename, &ext) {
                    Some(entry) => {
                        ...
                        match core::str::from_utf8(&file_data) {
                            ...
                            Err(_) => {
                                return Err(ProcessError {
                                    error_code: "Script contains invalid text formatting"
                                        .to_string(),
                                });
                            }
                        }
...
fn format_disk(args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
...
fn read_as_string(args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        if (args.len() != 2) {
            return Err(ProcessError {
...
fn read(args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        if (args.len() != 2) {
            return Err(ProcessError {
                error_code: "Needs 2 parameters".to_string(),
...
fn clear(_args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        WRITER.lock().clear();
        Ok(Success {
            success_code: "worked".to_string(),
...
fn pong(_args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        ...
        let mut game = crate::pong::PongGame::new();

        let result = game.run().await;

        crate::interrupts::LAUNCH_PONG.store(false, core::sync::atomic::Ordering::Relaxed);
        result
    })
}

fn history(_args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        let s = SHELL.lock();
        s.history();
        Ok(Success {
...
fn echo(args: Vec<String>) -> CommandFuture {
    Box::pin(async move {
        if args.len() < 2 {
            return Err(ProcessError {
...
pub fn run_cmd(cmd: Vec<String>) -> Result<CommandFuture, ProcessError> {
    if cmd.is_empty() {
        // Return a dummy future for an empty command
...
lazy_static::lazy_static! {
    static ref COMMANDS: Mutex<BTreeMap<String, Arc<dyn Fn(Vec<String>) -> CommandFuture + Send + Sync>>> =
